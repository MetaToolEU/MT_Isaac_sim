...
if [ "$input_arg" == "x86" ]; then
    ...
    docker run --name container_$input_arg --entrypoint bash -it --gpus all -e "ACCEPT_EULA=Y" --rm --network=host \
        ...
        -e "LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/isaac-sim/exts/omni.isaac.ros2_bridge/foxy/lib" \
        ...
        -v "$SCRIPT_DIR../../Isaac_launch/:/pkgs/curobo/examples/Isaac_launch:rw"\
