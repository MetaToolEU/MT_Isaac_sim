...
# Add ROS2 Foxy repository (correct URL for Ubuntu 20.04)
RUN curl -sSL https://raw.githubusercontent.com/ros/rosdistro/master/ros.key | apt-key add - && \
    echo "deb [arch=amd64] http://packages.ros.org/ros2/ubuntu focal main" | tee /etc/apt/sources.list.d/ros2.list && \
    apt-get update

# Install ROS2 Foxy
RUN apt-get install -y ros-foxy-desktop python3-colcon-common-extensions && \
    apt-get clean && rm -rf /var/lib/apt/lists/*

# Source ROS2 setup
RUN echo "source /opt/ros/foxy/setup.bash" >> ~/.bashrc

# Install additional ROS2 dependencies as needed
RUN apt-get update && \
    apt-get clean && rm -rf /var/lib/apt/lists/*
...
RUN mkdir -p /pkgs && \
    cd /pkgs && \
    git clone https://github.com/MetaToolEU/curobo.git -b isaac-3.0
